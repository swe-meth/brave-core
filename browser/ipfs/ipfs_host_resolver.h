...
class IPFSHostResolver : public network::ResolveHostClientBase {
 public:
  ...
  void SetResolverForTesting(network::mojom::HostResolver* test_host_resolver) {
    test_host_resolver_ = test_host_resolver;
  }

  void SetStopCallbackForTesting(base::OnceClosure stop_callback) {
    stop_called_callback_ = std::move(stop_callback);
  }
